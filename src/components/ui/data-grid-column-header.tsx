...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          "text-accent-foreground font-normal inline-flex h-full items-center gap-1.5 text-[0.8125rem] leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60 tr",
          className,
        )}
      >
        {icon && icon}
      ...
      >
        <div className="flex items-center gap-1">
          ...
          {filter && (
            <ListFilter
              className={cn(
                "size-3.5 ml-0.5 transition-all duration-300",
                isFiltered
    ...
    return (
      <div className="flex items-center h-full gap-1.5 justify-between">
        <DropdownMenu>
          ...
          <DropdownMenuContent
            className="min-w-64 w-max max-w-md"
            align="start"
          >
