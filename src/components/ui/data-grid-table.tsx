  ...
  return (
    <th
      ...
      className={cn(
        ...
        headerCellSpacing,
        props.tableLayout?.cellBorder && !isLastLeftPinned && "border-e",
    ...
    <div
      {...{
        ...
        className: cn(
          ...
          !props.tableLayout?.cellBorder && "before:bg-border",
        ),
      }}
    />
  );
...
function DataGridTableBodyRow<TData>({
  ...
  // Get custom row className from props if provided
  const customRowClassName = props.getRowClassName
  ...
  const handleRowClick = () => {
    ...
    // Call the onRowClick prop with the row data
    if (props.onRowClick) {
    ...
    <tr
      ...
      className={cn(
        ...
        props.tableLayout?.stripped &&
          "odd:bg-muted/90 hover:bg-transparent odd:hover:bg-muted",
        table.options.enableRowSelection && "[&_>:first-child]:relative",
        customRowClassName, // Add custom row className from props
        props.tableClassNames?.bodyRow,
      )}
    >
      {children}
  ...
  return (
    ...
    >
      <td colSpan={row.getVisibleCells().length}>
        <div className="grid grid-rows-[1fr] transition-[grid-template-rows] duration-300 ease-in-out">
