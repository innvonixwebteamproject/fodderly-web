...
export function StateForm({ initialData, onSubmit, onCancel, isLoading, viewOnly }: StateFormProps) {
  ...
  const handleAutoTranslate = async () => {
    ...
    if (!englishName) {
      toast.error("Please enter the English state name first.");
      return;
    }
          ...
          <Card className="h-fit">
            ...
            <CardContent className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel required>State Name</FormLabel>
                    <FormControl>
                      <Input {...field} placeholder="Enter state name" disabled={isLoading || viewOnly} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
